 ...
 *
 * The API stores and sends only the code (Destination.countryCode, and the
 * countryCodes on checklist items, plans and budget rows). Every place that
 * shows a country name — the Destinations table, the checklist, itinerary and
 * budget chips, the country picker — looks the code up here, so a name is
 * spelled once and cannot disagree with itself between tabs.
 *
 * The published page cannot import this file, so `npm run countries` writes
 * the same table to the API's publish/countries.json, which the renderer
 * inlines into every page. `npm run check` fails if the two have drifted.
 *
 * Names are the ones travellers use ("Bolivia", not "Bolivia (Plurinational
 * State of)"). Sorted by name, which is the order the picker lists them in.
 */
